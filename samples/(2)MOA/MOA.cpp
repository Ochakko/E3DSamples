// MOA.cpp : アプリケーションのエントリ ポイントを定義します。
// moaとはモーションアクセラレータの略です。詳しくはrdb2_MA.htmをご覧ください。
// １，２，３，４キーを押してください。
// モーションの変わり目で自動的に補間モーションが挿入されます。
                     ...
                     int       nCmdShow)
{
	...
	LoadString(hInstance, IDC_MOA, szWindowClass, MAX_LOADSTRING);
	...
	hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_MOA));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon			= LoadIcon(hInstance, MAKEINTRESOURCE(IDI_MOA));
	wcex.hCursor		= LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground	= (HBRUSH)(COLOR_WINDOW+1);
	wcex.lpszMenuName	= MAKEINTRESOURCE(IDC_MOA);
...
int OneTimeSceneInit()
{
	...
	sprintf_s( motname, MAX_PATH, "%s\\my\\my.moa", szMediaDir );
	...
	ZeroMemory( moainfo, sizeof( MOAINFO ) * MAXMOTNUM );
	ret = E3DGetMoaInfo( hsid1, MAXMOTNUM, moainfo, &migetnum );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	}
...
int Render3DEnvironment()
{
	...
	ret = E3DSetNewPoseByMOA( hsid1, eventno );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	}
...
int DrawText()
{
	...
	texpos.y += 20;	
	ret = E3DGetMotionFrameNo( hsid1, &curmotid, &curframe );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	}
	sprintf_s( mes, 1024, "motionID %d, frameno %d\r\n", curmotid, curframe );
	ret = E3DDrawText( texpos, 1.0f, texcol, mes );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	}
	...
	texpos.y += 20;
	ret = E3DGetNextMotionFrameNo( hsid1, curmotid, &nextmotid, &nextframe );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	}
	sprintf_s( mes, 1024, "nextmotionID %d, nextframeno %d\r\n", nextmotid, nextframe );
	ret = E3DDrawText( texpos, 1.0f, texcol, mes );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	...
	texpos.y += 20;
	sprintf_s( mes, 1024, "migetnum : %d", migetnum );
	ret = E3DDrawText( texpos, 1.0f, texcol, mes );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	...
	POINT texpos2;
	texpos2.x = 0;
	texpos2.y = (LONG)texpos.y;
	int i;
	for( i = 0; i < migetnum; i++ ){
		texpos2.y += 20;
		sprintf_s( mes, 1024, "name : %s, id : %d", moainfo[i].name, moainfo[i].id );
		E3DDrawTextByFontID( scid, fontid1, texpos2, mes, texcol );
		if( ret ){
			_ASSERT( 0 );
			return 1;
		}
