// CustomShader.cpp : アプリケーションのエントリ ポイントを定義します。
//
                     ...
                     int       nCmdShow)
{
	...
	LoadString(hInstance, IDC_CUSTOMSHADER, szWindowClass, MAX_LOADSTRING);
	...
	hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_CUSTOMSHADER));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon			= LoadIcon(hInstance, MAKEINTRESOURCE(IDI_CUSTOMSHADER));
	wcex.hCursor		= LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground	= (HBRUSH)(COLOR_WINDOW+1);
	wcex.lpszMenuName	= MAKEINTRESOURCE(IDC_CUSTOMSHADER);
...
int OneTimeSceneInit()
{
	...
	ret = E3DCreateRenderTargetTexture( texsize, D3DFMT_A8R8G8B8, &scid_final, &texid_final, &okflag );
	if( ret || !okflag ){
		_ASSERT( 0 );
		return 1;
	}
	char bgname[MAX_PATH];
	sprintf_s( bgname, MAX_PATH, "%s\\lake.bmp", szMediaDir );
	ret = E3DCreateBG( scid_org, bgname, "dummy", 0.0f, 0.0f, 0, proj_far );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	}
	...
	fl4_0 = D3DXVECTOR4( 0.5f, 1.0f, 1.0f, 1.0f );
...
int Render3DEnvironment()
{
	...
	ret = E3DShaderConstUserFL4( 0, fl4_0 );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	}
	...
	//shaderのusertex0をセット
	ret = E3DShaderConstUserTex( 0, texid_org );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	}

	//shaderのPostEffectUser5テクニックのパス０を実行
	ret = E3DCallUserShader( 5, 0, scid_final );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	}
