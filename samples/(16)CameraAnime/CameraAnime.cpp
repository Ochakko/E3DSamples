// CameraAnime.cpp : アプリケーションのエントリ ポイントを定義します。
//
                     ...
                     int       nCmdShow)
{
	...
	LoadString(hInstance, IDC_CAMERAANIME, szWindowClass, MAX_LOADSTRING);
	...
	hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_CAMERAANIME));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon			= LoadIcon(hInstance, MAKEINTRESOURCE(IDI_CAMERAANIME));
	wcex.hCursor		= LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground	= (HBRUSH)(COLOR_WINDOW+1);
	wcex.lpszMenuName	= MAKEINTRESOURCE(IDC_CAMERAANIME);
...
int OneTimeSceneInit()
{
	...
	sprintf_s( animname, MAX_PATH, "%s\\cameraanim_1.cam", szMediaDir );
	ret = E3DLoadCameraFile( animname, &animno1 );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	}
	sprintf_s( animname, MAX_PATH, "%s\\cameraanim_2.cam", szMediaDir );
	ret = E3DLoadCameraFile( animname, &animno2 );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	}

	ret = E3DSetCameraAnimNo( animno1 );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	}
//	ret = E3DDeleteCameraKey( animno1, 2, 59 );
//	if( ret ){
//		_ASSERT( 0 );
//		return 1;
//	}
	E3DCAMERASTATE cs;
	cs.looktype = CAML_NORMAL;
	cs.pos = D3DXVECTOR3( 1926.0f, 663.0f, 2116.0f );
	cs.target = D3DXVECTOR3( 0.0f, 750.0f, 0.0f );
	D3DXVECTOR3 diff = cs.pos - cs.target;
	cs.dist = D3DXVec3Length( &diff );
	cs.nearz = 100.0f;
	cs.farz = 40000.0f;
	cs.fov = 45.0f;
	cs.hsid = -1;
	cs.boneno = -1;
	cs.ortho = 0;
	cs.orthosize = 3000.0f;
	ret = E3DSetCameraKey( animno1, 2, 59, cs );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	}
	
	ret = E3DSetNextCameraAnimFrameNo( animno1, animno2, 0, -1 );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	}
	ret = E3DSetNextCameraAnimFrameNo( animno2, animno1, 0, -1 );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	}
...
int DrawText()
{
	...
	int ret;
	int curanim, curcamera, curframe;
	ret = E3DGetCameraFrameNo( &curanim, &curcamera, &curframe );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	}
	...
	texpos.y += 20;
	sprintf_s( mes, 1024, "animno %d, camerano %d, frameno %d", curanim, curcamera, curframe );
