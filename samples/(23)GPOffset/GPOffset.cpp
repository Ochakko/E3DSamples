// GPOffset.cpp : アプリケーションのエントリ ポイントを定義します。
...
int charaqid, invcharaqid, camqid, gpqid;
D3DXVECTOR3 aftdiff;
                     ...
                     int       nCmdShow)
{
	...
	LoadString(hInstance, IDC_GPOFFSET, szWindowClass, MAX_LOADSTRING);
	...
	hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_GPOFFSET));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon			= LoadIcon(hInstance, MAKEINTRESOURCE(IDI_GPOFFSET));
	wcex.hCursor		= LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground	= (HBRUSH)(COLOR_WINDOW+1);
	wcex.lpszMenuName	= MAKEINTRESOURCE(IDC_GPOFFSET);
...
int OneTimeSceneInit()
{
	...
	inipos1 = D3DXVECTOR3( 0.0f, 0.0f, 0.0f );
	pos1 = inipos1;
	...
	sprintf_s( camname1, MAX_PATH, "%s\\movetest3.cam", szMediaDir );
	...
	gptra = D3DXVECTOR3( 0.0f, 0.0f, 0.0f );
	gpdiff = D3DXVECTOR3( 0.0f, 0.0f, 0.0f );

	char gpname1[MAX_PATH];
	sprintf_s( gpname1, MAX_PATH, "%s\\initial.gpa", szMediaDir );
	ret = E3DLoadGPFile( hsid1, gpname1, &gpanim0 );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	}
//	E3DSetGPGroundHSID( hsid1, gpanim0, hsid0 );
	...
	char gpname2[MAX_PATH];
	sprintf_s( gpname2, MAX_PATH, "%s\\movetest3.gpa", szMediaDir );
	ret = E3DLoadGPFile( hsid1, gpname2, &gpanim1 );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	}
//	E3DSetGPGroundHSID( hsid1, gpanim1, hsid0 );
	...
	ret = E3DSetGPAnimNo( hsid1, gpanim0 );
	_ASSERT( !ret );
...
int MoveChara()
{
	...
	if( (changeflag == 1) && (camerastate == CAMERA_NORMAL) ){
		gptra = pos1 - inipos1;
		ret = E3DSetGPOffset( hsid1, gptra, charaqid );
		_ASSERT( !ret );
		ret = E3DCalcGPFullFrame( hsid1, gpanim0 );
		_ASSERT( !ret );
	}
...
int MoveCameraAndGP()
{
	...
	if( camerastate == CAMERA_SETUP ){
		if( cameraframe == 0 ){
			...
			gpdiff = (pos1 - gptra) / (float)SETUPTIME;
		}
		camoffbef = -pos1;
		camoffaft += aftdiff;
		...
		gptra += gpdiff;
		ret = E3DSlerpQ( gpqid, charaqid, t, gpqid );
		_ASSERT( !ret );

		cameraframe++;

		ret = E3DSetGPOffset( hsid1, gptra, gpqid );
		_ASSERT( !ret );
		ret = E3DCalcGPFullFrame( hsid1, gpanim0 );
		_ASSERT( !ret );
		...
		if( cameraframe >= SETUPTIME ){
			...
			ret = E3DCalcGPFullFrame( hsid1, gpanim1 );
			_ASSERT( !ret );
			ret = E3DSetGPAnimNo( hsid1, gpanim1 );
			_ASSERT( !ret );
