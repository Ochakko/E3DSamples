// Mono.cpp : アプリケーションのエントリ ポイントを定義します。
//
                     ...
                     int       nCmdShow)
{
	...
	LoadString(hInstance, IDC_MONO, szWindowClass, MAX_LOADSTRING);
	...
	hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_MONO));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon			= LoadIcon(hInstance, MAKEINTRESOURCE(IDI_MONO));
	wcex.hCursor		= LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground	= (HBRUSH)(COLOR_WINDOW+1);
	wcex.lpszMenuName	= MAKEINTRESOURCE(IDC_MONO);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   UpdateWindow(hWnd);
	...
	E3DCOLOR4UC clearcol = { 0, 255, 255, 255 };
...
int OneTimeSceneInit()
{
	...
	ret = E3DCreateRenderTargetTexture( texsize, D3DFMT_A8R8G8B8, &scid_mono, &texid_mono, &okflag );
	if( ret || !okflag ){
		_ASSERT( 0 );
		return 1;
	}
	...
	ret = E3DSetTextureToDispObj( -2, spid2, texid_mono );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	}
...
int Render3DEnvironment()
{
	...
	ret = E3DMonoRTTexture( scid_mono, texid_org );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	}
